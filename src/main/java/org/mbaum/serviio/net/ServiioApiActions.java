...
public final class ServiioApiActions
{
	...
	public static final Process<RepositoryContext, MutableModel<RepositoryModel>> REPOSITORY_PROCESS = createRepositoryProcess();
    ...
    {
	    return new HttpProcess<ActionContext, PingResponse>( newJsonParser( PingResponse.class ), "action" )
		{
			@Override
            protected HttpUriRequest buildRequest( ActionContext context )
                    throws Exception
            {
				HttpEntity input = createJsonInputString( context.getAction() );
				
				return RequestBuilder.post().setUri( buildUri( context.getServiioModel(), ACTION_PATH ) )
	...
	{
		return new HttpProcess<PingContext, PingResponse>( newJsonParser( PingResponse.class ), "ping" )
		{
			@Override
	...
	{
		return new HttpProcess<RepositoryContext, MutableModel<RepositoryModel>>( newMutableModelJsonParser( RepositoryModel.class ), "repository" )
		{
			@Override
	...
	{
		return new HttpProcess<RepositoryContext, PingResponse>( newJsonParser( PingResponse.class ), "repository" )
		{
			@Override
			protected HttpUriRequest buildRequest( RepositoryContext context )
					throws Exception
			{
			    MutableModel<RepositoryModel> repositoryResponse = context.getServiioModel().getValue( REPOSITORY_RESPONSE );
				
				List<MutableModel<OnlineRepositoryModel>> onlineRepositories = repositoryResponse.getValue( RepositoryModel.ONLINE_REPOSITORIES );
...
//				newRepo.setContentUrl( "http://hscontent.com/east13/OTHER_HSTV_CHIHD.m3u8?token=c8ncvLqnxOIDq7I8WafBaKi3dK6FBsBeXBhl/9vZvYdgCKnwgHjT+9JUBVbtjr74slnQCbdstYdoNBtRcMEu" );
//				newRepo.setRepositoryType( "LIVE_STREAM" );
//				onlineRepositories.add( newRepo );
				
				return buildPutRequest( context.getServiioModel(), REPOSITORY_PATH, repositoryResponse );
			}
		};
	}
	    ...
	    throws URISyntaxException, JsonGenerationException, JsonMappingException, IOException, JAXBException
	{
		...
		return RequestBuilder.put().setUri( buildUri( serviioModel, path ) )
				                   .addHeader( "Accept", "application/json" )
				                   .addHeader( "Content-Type", "application/json" )
				                   .setEntity( input )
				                   .build();
	...
	private static <T> HttpEntity createJsonInputString( T data ) 
